...
- The client uses TCP sockets to send data to the server using the write()
system call. TCP is a reliable, stream-based protocol.

2. What data is the client sending to the server?
- The client sends a predefined set of short string messages (e.g., "Hello",
"Apple", etc.) in a loop, with small delays between sends.

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
- `run_acceptor` is passed a pointer to a `struct acceptor_args` which includes
a pointer to the shared message list(list_handle) and associated mutex
(list_lock) for thread-safe access

2. How are received messages stored?
- Messages are stored in a global linked list, where each message is
encapsulated in a `list_node` struct. New nodes are added by the client threads
using the `add_to_list()` function.

3. What does `main()` do with the received messages?
- `main()` waits until a predefined number of messages are received (MAX_CLIENTS
× NUM_MSG_PER_CLIENT), and then prints all collected messages from the list.

4. How are threads used in this sample code?
- One thread (`run_acceptor`) accepts new client connections and creates a new
thread for each one. Each client thread (`run_client`) receives messages from a
single client and stores them.

Explain the use of non-blocking sockets in this lab:
- Non-blocking sockets are used so that `read()` does not block the
client-handling thread if no data is available, allowing it to remain responsive
and check again later

How are sockets made non-blocking?
- The client sockets are made non-blocking by calling `fcntl()` with the
`O_NONBLOCK` flag after `accept()`.

What sockets are made non-blocking?
- Only the per-client sockets (the ones returned by `accept()`) are set to
non-blocking. The main server socket remains blocking.

Why are these sockets made non-blocking? What purpose does it serve?
- Non-blocking sockets allow the server to attempt reading data without getting
stuck if no data is currently available. This enables the client thread to
remain responsive and eventually terminate cleanly when needed.
*/
...
static void *run_acceptor(void *args) {
  ...
  for (int i = 0; i < num_clients; i++) {
    ...
    client_args[i].run = false;     // stop signal
    pthread_join(threads[i], NULL); // wait for thread to be done
    close(client_args[i].cfd);      // client socket close
  }
...
int main() {
  ...
  while (1) {
    ...
    pthread_mutex_unlock(&list_mutex);
    usleep(1000); // small sleep for safety
  }
